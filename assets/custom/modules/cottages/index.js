$(function () {
	...
	$('.view-row').on('click', function(e) {
		e.preventDefault();
		clearValueControls([$('#view_id'), $('#view_name'), $('#view_description'), $('#view_no_of_guests'), $('#view_price'), $('#view_slots')]);
	...
	viewDetailsModalEl.on('shown.bs.modal', function (e) {
		e.preventDefault();
		$.ajax({
			url: `${BASE_URL}cottages/get/${$('#view_id').val()}`,
			type: 'GET',
			async: true,
			data: null,
			dataType: 'json',
